...
func (g *Generator) Parse(b []byte, ifaces ...string) ([]byte, error) {
...
func (g *Generator) Generate(ifaces ...introspect.Interface) ([]byte, error) {
	buf := &buffer{}
...
`, g.pkgName)
	...
	for _, iface := range ifaces {
		g.generate(buf, iface)
	}
	return format.Source(buf.Bytes())
}

func (g *Generator) generate(buf *buffer, iface introspect.Interface) error {
	typ := nameToGoType(iface.Name, true)
	buf.WriteStringf(`func New%s(conn *dbus.Conn, dest string, path dbus.ObjectPath) *%s {
	return &%s{conn.Object(dest, path)}
}

`, typ, typ, typ)
	buf.WriteStringf(`type %s struct {
...
`, typ)
	...
	for _, method := range iface.Methods {
		...
		buf.WriteStringf(`func(o *%s) %s(%s) (%serr error) {
...
func argsToGoInOut(args []introspect.Arg) ([]arg, []arg) {
	var in, out []arg
	for i := range args {
		name := varNameToGo(args[i].Name, i)
		o := arg{name, sigToGo(args[i].Type, ",")}
...
func varNameToGo(name string, i int) string {
	if name == "" {
		return "arg" + strconv.Itoa(i)
	} else if isKeyword(name) {
		return "arg" + strings.Title(name)
	}
